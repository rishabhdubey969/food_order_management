...
export class OtpService {
  /**
   * Initializes the OtpService with dependencies for Redis, token handling, and logging.
   *
   * Args:
   *   redisService (RedisService): Service for storing and retrieving OTPs in Redis.
   *   tokenService (TokenService): Service for hashing and comparing OTPs.
  ...
  async verify(partnerEmail: string, otp: string) {
    /**
     * Verifies an OTP for a given email and generates an access token upon successful verification.
     *
     * Args:
     *   partnerEmail (string): The email address associated with the OTP.
     *   otp (string): The OTP provided by the user for verification.
     *
     * Returns:
     *   Promise<{ accessToken: string }>: An object containing the generated access token.
     *
     * Throws:
     *   ForbiddenException: If the OTP is not found or has expired.
     *   BadRequestException: If the provided OTP is invalid.
     *   InternalServerErrorException: If there is an error retrieving the OTP from Redis.
     */
  ...
  async generateOtp(email: string): Promise<string> {
    /**
     * Generates a random OTP for a given email and stores its hashed value in Redis.
     *
     * Args:
     *   email (string): The email address to associate with the OTP.
     *
     * Returns:
     *   Promise<string>: The generated OTP as a string.
     */
