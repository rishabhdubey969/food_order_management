...
export class DeliveryController {
  ...
  async handleOrderPickup(
    ...
    /**
     * Handles the order pickup event by updating the delivery status to PICKED_UP and scheduling an IN_TRANSIT update.
     *
     * Args:
     *   orderId (Types.ObjectId): The unique identifier of the order being picked up.
     *   context (KafkaContext): The Kafka context containing consumer and message details.
     *
     * Returns:
     *   Promise<void>: No return value; updates the delivery status and commits Kafka offsets.
     */
    this.logger.info('Processing order pickup event', {
      service: 'DeliveryController',
      method: 'handleOrderPickup',
  ...
  async createDelivery(
    ...
    /**
     * Creates a new delivery for a given order in response to a new order event.
     *
     * Args:
     *   data ({ orderId: Types.ObjectId }): The payload containing the order ID.
     *   context (KafkaContext): The Kafka context containing consumer and message details.
     *
     * Returns:
     *   Promise<void>: No return value; creates the delivery and commits Kafka offsets.
     *
     * Throws:
     *   Error: If the delivery creation process fails.
     */
    const { orderId } = data;
    this.logger.info('Processing new order event', {
      service: 'DeliveryController',
  ...
  async handleOrderDelivered(@Body('orderId') orderId: Types.ObjectId) {
    /**
     * Marks an order as delivered by updating its delivery status.
     *
     * Args:
     *   orderId (Types.ObjectId): The unique identifier of the order to mark as delivered.
     *
     * Returns:
     *   Promise<{ success: boolean, message: string, data: { orderId: Types.ObjectId, status: DeliveryStatus, deliveredAt: string } }>:
     *     A success response with delivery details.
     */
    this.logger.info('Processing order delivered request', {
      service: 'DeliveryController',
      method: 'handleOrderDelivered',
  ...
  async getPartnerDeliveries(
    ...
    /**
     * Retrieves the delivery history for a specific delivery partner with pagination.
     *
     * Args:
     *   partnerId (Types.ObjectId): The unique identifier of the authenticated partner.
     *   page (number): The page number for pagination.
     *   limit (number): The number of deliveries to return per page.
     *
     * Returns:
