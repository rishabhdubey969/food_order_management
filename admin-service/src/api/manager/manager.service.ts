...
interface ManagerServiceGrpc {
  signupManager(data: GetSignUpRequest): Observable<GetSignUpResponse>;
 DeleteManager (data:DeleteManagerRequest):Observable<DeleteManagerResponse>;
 GetAllManagers (data:GetAllManagersRequest):Observable<GetAllManagersResponse>;
}
...
export interface GetAllManagersRequest {
token :string;
}
export interface GetAllManagersResponse {
   message :string;
...
export class ManagerService implements OnModuleInit {
  ...
  async signup(data :GetSignUpRequest) {
    return this.managerServiceGrpc.signupManager(data)
  }

  async deletemanager(data:DeleteManagerRequest){
    return this.managerServiceGrpc.DeleteManager(data);
  }
  async getallmanager(data:GetAllManagersRequest){
    return this.managerServiceGrpc.GetAllManagers(data);
  }
  async blockManagerAndRestaurant(managerId: string, restaurantId: string) {
    this.logger.log(
      `Attempting to block manager ${managerId} and restaurant ${restaurantId}`,
    );
    try {
      // Validate manager
      const manager = await this.connection
        .collection('managers')
        .findOne({ _id: new ObjectId(managerId), isdeleted: false });
      if (!manager) {
      ...
      if (!restaurant) {
        this.logger.warn(
          `Restaurant ${restaurantId} not found or not associated with manager ${managerId}`,
        );
        throw new HttpException(
          'Restaurant not found or not associated with this manager',
      ...
      if (updates.length === 0) {
        this.logger.warn(
          `Manager ${managerId} and restaurant ${restaurantId} are already blocked`,
        );
        throw new HttpException(
      ...
      try {
        ...
        this.logger.log(
          `Block notification email emitted for ${manager.email}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to emit block notification email: ${error.message}`,
          error.stack,
        );
      }
      ...
      this.logger.error(
        `Failed to block manager ${managerId} and restaurant ${restaurantId}: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
  ...
  async unblockManagerAndRestaurant(
    ...
    this.logger.log(
      `Attempting to unblock manager ${managerId} and restaurant ${restaurantId} by admin: ${adminId}`,
    );
    try {
      // Validate manager
      const manager = await this.connection
      ...
      if (!restaurant.is_active) {
        ...
        this.logger.log(
          `Restaurant ${restaurantId} is already unblocked, skipping`,
        );
      }

      if (updates.length === 0) {
        this.logger.warn(
          `Manager ${managerId} and restaurant ${restaurantId} are already unblocked`,
        );
        throw new HttpException(
          'Manager and restaurant are already unblocked',
          HttpStatus.BAD_REQUEST,
        );
      }
      ...
      try {
        ...
        this.logger.log(
          `Unblock notification email emitted for ${manager.email}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to emit unblock notification email: ${error.message}`,
          error.stack,
        );
      }
      ...
      this.logger.error(
        `Failed to unblock manager ${managerId} and restaurant ${restaurantId}: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
  ...
  async getAllManagers(token: string, page: number = 1, limit: number = 10) {
    ...
    try {
      ...
      try {
        ...
        this.logger.error(
          `Pagination parameter validation failed: ${error.message}`,
          error.stack,
        );
        throw new HttpException(
      ...
      try {
        ...
        this.logger.error(
          `Manager query failed: ${error.message}`,
          error.stack,
        );
        throw new HttpException(
      ...
      try {
        ...
        this.logger.error(
          `Pagination calculation failed: ${error.message}`,
          error.stack,
        );
        throw new HttpException(
          'Failed to fetch managers',
