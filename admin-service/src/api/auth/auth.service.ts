  ...
  try {
    ...
    try {
      try {
        if (admin.deviceId !== deviceId) {
          ...
          try {
            ...
            this.logger.error(`Failed to emit new device email event: ${emailError.message}`, emailError.stack);
          }
        }
      } catch (error) {
        this.logger.error(`Device ID check failed: ${error.message}`, error.stack);
      }
      ...
      try {
        ...
        this.logger.error(`OTP generation/save failed: ${error.message}`, error.stack);
      ...
      try {
        ...
        this.logger.error(`Failed to emit OTP email event: ${error.message}`, error.stack);
        throw new HttpException('Admin login failed', HttpStatus.INTERNAL_SERVER_ERROR);
      }
    } catch (error) {
      this.logger.error(`Email setup failed: ${error.message}`, error.stack);
      throw new HttpException('Admin login failed', HttpStatus.INTERNAL_SERVER_ERROR);
    }
 ...
 async verifyOtp(verifyOtpDto: VerifyOtpDto) {
  ...
  try {
    ...
    try {
      ...
      this.logger.error(`OTP check failed: ${error.message}`, error.stack);
      throw new HttpException('Invalid or expired OTP', HttpStatus.BAD_REQUEST);
    }
  ...
  async logout(userId: string) {
    ...
    try {
      let admin;
      try {
        admin = await this.connection.collection("admins")
          .findOne({ _id: new ObjectId(userId), role: 0, is_active: true, is_deleted: false })
      ...
      try {
        ...
        throw new HttpException('Admin logout failed', HttpStatus.INTERNAL_SERVER_ERROR);
      }
    } catch (error) {
...
async resetPassword(resetPasswordDto: ResetPasswordDto) {
  ...
  try {
    ...
    let payload;
    try {
      payload = this.jwtService.verify(token); // Directly verify the token
      if (!payload.sub) {
    ...
    let adminId;
    try {
      const redisKey = `reset:${payload.sub}`;
      const storedToken = await this.redisClient.get(redisKey);
      if (!storedToken) {
        this.logger.warn(`Reset token not found in Redis for admin ID: ${payload.sub}`);
        throw new HttpException('Invalid or expired reset token', HttpStatus.BAD_REQUEST);
      }
      if (storedToken !== token) {
        this.logger.warn(`Reset token mismatch for admin ID: ${payload.sub}`);
        throw new HttpException('Invalid reset token', HttpStatus.BAD_REQUEST);
      }
      adminId = payload.sub;
    } catch (error) {
    ...
    // Step 6: Delete the token from Redis
    try {
      const redisKey = `reset:${adminId}`;
      await this.redisClient.del(redisKey);
      this.logger.log(`Deleted reset token from Redis for admin ID: ${adminId}`);
    } catch (error) {
      this.logger.error(`Failed to delete reset token from Redis: ${error.message}`, error.stack);
      // Non-critical failure; password reset has already succeeded
    }
