  ...
  async login(loginDto: LoginDto) {
    ...
    try {
      ...
      const admin = await this.connection
        .collection('admins')
        .findOne({ email, role: 0, is_active: true, is_deleted: false }, { projection: { password: 1, deviceId: 1 } });
console.log(admin)
      ...
      throw new HttpException(error.message || 'Admin login failed', error.status || HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async verifyOtp(verifyOtpDto: VerifyOtpDto) {
    ...
    try {
      ...
      await this.connection
        .collection('admins')
        .updateOne({ _id: new ObjectId(userId) }, { $unset: { otp: '', otpExpires: '' } });
      ...
      await this.redis.set(`access_token:${admin._id}:${accessToken}`, JSON.stringify(payload), 'EX', 10 * 60);
  ...
  async refreshToken(refreshToken: string) {
    ...
    try {
      ...
      await this.redis.set(`access_token:${payload.sub}:${newAccessToken}`, JSON.stringify(payload), 'EX', 10 * 60);
  ...
  async logout(userId: string) {
    ...
    try {
      ...
      throw new HttpException(error.message || 'Admin logout failed', error.status || HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async forgotPassword(forgotPasswordDto: ForgotPasswordDto) {
    ...
    try {
      ...
      const admin = await this.connection.collection('admins').findOne({ email, role: 0, is_active: true });
      if (!admin) {
        this.logger.warn(`Admin not found for email: ${email}`);
      ...
      if (existingToken) {
        ...
        if (admin.resetToken && admin.resetTokenExpires > new Date()) {
          ...
          resetToken = this.jwtService.sign({ sub: admin._id, email: admin.email }, { expiresIn: '1h' });
          const updateResult = await this.connection.collection('admins').updateOne(
            { _id: admin._id },
            {
              $set: {
